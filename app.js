...
const defaultServices = [
  ...
  { id: 'spine', name: '脊柱功能康复', detail: '60 分钟 · 手法治疗与动作训练', price: '使用次数', icon: 'activity' },
  { id: 'training', name: '运动损伤治疗', detail: '60 分钟 · 疼痛管理与功能恢复', price: '使用次数', icon: 'dumbbell' },
];
...
const defaultWorkspacePatients = [
  {
    ...
    stage: '第 1 阶段 · 疼痛控制', done: 2, total: 8, painBefore: 7, painNow: 4, nextFocus: '骨盆控制、髋屈肌放松与死虫式训练。',
    history: [
      { date: '2026.08.21', title: '第 2 次治疗 · 骨盆稳定', content: '站立耐受时间提升至 40 分钟；久坐后仍有紧张感，已调整办公间歇策略。' },
      { date: '2026.08.14', title: '初次评估 · 腰背疼痛', content: '右侧腰方肌紧张明显，久坐疼痛评分 7 / 10；建立疼痛记录与居家活动计划。' },
    ],
  },
  {
    id: 'jiang', name: '蒋远', profile: '28 岁 · 跑步爱好者', therapistId: 'li', demand: '跑步后右膝外侧疼痛，目标是在秋季赛事前安全恢复 10 公里训练。',
    stage: '初次评估', done: 0, total: 6, painBefore: 6, painNow: 6, nextFocus: '跑姿筛查、髋外展力量测试与训练负荷调整。',
    history: [
      { date: '2026.08.23', title: '预约前问卷', content: '疼痛主要出现在跑步 5 公里后，下坡与久坐起身时较明显；暂未进行系统治疗。' },
    ],
  },
  {
    ...
    history: [
      { date: '2026.08.20', title: '第 5 次治疗 · 动态稳定', content: '单脚平衡由 12 秒提升至 28 秒；低强度侧向移动无明显不适。' },
      { date: '2026.08.13', title: '第 4 次治疗 · 负重控制', content: '踝关节背屈活动度改善，开始加入轻阻力提踵与弹力带外翻。' },
      { date: '2026.07.30', title: '初次评估 · 踝关节扭伤恢复', content: '距上关节活动受限，运动后不稳感明显，制定阶段性功能恢复安排。' },
    ],
  },
];
...
const defaultPatientConcerns = {
  ...
  wu: [
    { id: 'wu-back', title: '久坐后腰背酸紧', summary: '久坐和通勤后明显，希望稳定完成每周两次力量训练。', status: 'active', priority: 'primary', openedAt: '2026.08.14', updatedAt: '2026.08.21', closeReason: '', statusHistory: [] },
  ...
  jiang: [
    { id: 'jiang-knee', title: '跑步后右膝外侧疼痛', summary: '约跑步 5 公里后出现，下坡与久坐起身时较明显。', status: 'active', priority: 'primary', openedAt: '2026.08.23', updatedAt: '2026.08.23', closeReason: '', statusHistory: [] },
  ],
  zhang: [
    { id: 'zhang-ankle', title: '左踝扭伤后不稳', summary: '目标是恢复周末羽毛球和变向移动信心。', status: 'active', priority: 'primary', openedAt: '2026.07.30', updatedAt: '2026.08.20', closeReason: '', statusHistory: [] },
    { id: 'zhang-knee', title: '右膝上下楼偶发不适', summary: '目前没有持续疼痛，保留复查观察。', status: 'monitoring', priority: 'secondary', openedAt: '2026.05.18', updatedAt: '2026.07.30', closeReason: '', statusHistory: [] },
  ],
};
...
function showAuth(preferredRole = state.authRole || 'customer') {
  ...
  if (auth.loggedIn) {
    ...
    const healthConsent = auth.role === 'customer'
      ? `<div class="auth-consent-summary ${auth.sensitiveConsent ? 'granted' : ''}"><i class="icon icon-${auth.sensitiveConsent ? 'shield-check' : 'shield-alert'}"></i><span><strong>健康信息单独授权</strong><small>${auth.sensitiveConsent ? '已授权，可建立评估和康复记录' : '未授权，建立评估前需要单独确认'}</small></span></div><button class="button secondary full" data-action="show-consent-records"><i class="icon icon-clipboard-list"></i> 查看授权记录</button>`
      : '';
...
function workspaceSchedule() {
  const currentService = state.booking?.service?.name || '脊柱功能康复';
  ...
  return [
    ...
    { time: '14:00', patientId: 'wu', therapistId: 'zhou', service: '腰背疼痛治疗', status: '已确认', tone: 'confirmed' },
    { time: '15:30', patientId: 'jiang', therapistId: 'li', service: '初次评估', status: '待评估', tone: 'pending' },
    { time: '17:00', patientId: 'zhang', therapistId: 'chen', service: '运动损伤治疗', status: '治疗中', tone: 'active' },
...
function adminAnalyticsData() {
  ...
  const historicalSchedule = [
    ...
    { date: '2026-08-22', time: '15:00', patientId: 'zhang', therapistId: 'chen', service: '运动损伤治疗', status: '已完成', tone: 'done' },
    { date: '2026-08-21', time: '14:00', patientId: 'wu', therapistId: 'zhou', service: '腰背疼痛治疗', status: '已完成', tone: 'done' },
    { date: '2026-08-21', time: '16:30', patientId: 'lin', therapistId: 'li', service: '脊柱功能康复', status: '已完成', tone: 'done' },
    { date: '2026-08-20', time: '11:00', patientId: 'zhang', therapistId: 'chen', service: '运动损伤治疗', status: '已完成', tone: 'done' },
    { date: '2026-08-19', time: '10:00', patientId: 'jiang', therapistId: 'li', service: '初次评估', status: '已完成', tone: 'done' },
    { date: '2026-08-18', time: '15:30', patientId: 'wu', therapistId: 'zhou', service: '腰背疼痛治疗', status: '已完成', tone: 'done' },
    { date: '2026-08-12', time: '10:30', patientId: 'lin', therapistId: 'li', service: '脊柱功能康复', status: '已完成', tone: 'done' },
    { date: '2026-08-08', time: '16:00', patientId: 'zhang', therapistId: 'chen', service: '运动损伤治疗', status: '已完成', tone: 'done' },
    { date: '2026-08-05', time: '14:30', patientId: 'wu', therapistId: 'zhou', service: '腰背疼痛治疗', status: '已完成', tone: 'done' },
...
function exportPatientsCsv() {
  ...
  downloadCsv(
    ...
    ['客户ID', '姓名', '基础资料', '负责康复师', '问题ID', '问题主诉', '问题说明', '问题优先级', '问题状态', '开始日期', '最近更新', '关闭日期', '关闭原因', '结果摘要', '已到店次数', '服务总次数', '剩余次数', '最近记录日期', '最近记录标题'],
    rows,
  );
}
...
function renderAdmin() {
  ...
  const patientRows = patients.map((patient) => {
    ...
    return `<button class="admin-patient-row" data-action="open-patient" data-patient="${patient.id}">
      ...
      <span class="admin-patient-concern"><strong>${escapeHtml(primary?.title || '暂无主诉')}</strong><small>最近记录 ${escapeHtml(patient.history?.[0]?.date || '暂无')}</small></span>
  ...
  el('#admin-content').innerHTML = `
    ...
    <section class="admin-analysis-section">
      <div class="admin-section-heading"><div><span class="section-kicker">ISSUE STATUS · ${analytics.period.shortLabel.toUpperCase()}</span><h2>问题状态分布</h2><p>按${analytics.period.shortLabel}内更新过的客户问题主诉统计。</p></div><strong class="admin-total">${concerns.length}<small>本期问题</small></strong></div>
    ...
    <section class="admin-settings">
      ...
      <details class="admin-settings-group"><summary><span><i class="icon icon-stethoscope"></i><strong>治疗项目</strong><small>${services.length} 个可预约项目</small></span><i class="icon icon-chevron-down"></i></summary><div class="admin-settings-body"><div class="admin-inline-head"><p>客户预约页会以这里的项目为准。</p><button class="icon-button" data-action="add-service" title="新增治疗项目" aria-label="新增治疗项目"><i class="icon icon-plus"></i></button></div><div class="editor-list">${serviceForms}</div></div></details>
...
function showSensitiveConsent(nextAction = 'assessment') {
  modal('健康信息单独授权', `<section class="sensitive-consent"><div class="sensitive-consent-icon"><i class="icon icon-shield-alert"></i></div><div><span class="section-kicker">SENSITIVE HEALTH DATA</span><h3>建立康复档案前请单独确认</h3><p>为进行评估、形成康复安排和记录治疗进展，需要处理你的健康信息。</p></div><section class="sensitive-consent-list"><strong>本次会处理</strong><span>主要诉求、既往不适与受伤情况</span><span>姿态/功能评估、疼痛与训练反馈</span><span>治疗记录、康复阶段及相关附件</span></section><section class="sensitive-consent-list"><strong>使用范围</strong><span>仅用于本工作室的预约服务、康复评估与治疗跟进；仅授权管理员及负责康复师按权限查看。</span></section><p class="sensitive-consent-note">你可在“账户与登录 - 授权记录”撤回授权。撤回后将停止新增健康记录；已保存信息的后续处理应以适用法律和服务所需为准。</p><button class="button primary full" data-action="grant-sensitive-consent" data-next="${nextAction}"><i class="icon icon-shield-check"></i> 同意并继续建档</button><button class="button secondary full" data-action="close-modal-button">暂不授权</button></section>`, 'sensitive-consent-modal');
}
...
function showAppointments() {
  const current = state.booking ? `<article class="appointment-card"><div class="date-badge"><span>${state.booking.date.day}</span><b>${state.booking.date.date.slice(-2)}</b></div><div><h3>${state.booking.service.name}</h3><p>${state.booking.time} · ${state.booking.therapist.name}<br />滨江康复中心 · 2F 治疗区</p></div><span class="state-pill active">已确认</span></article>` : '<div class="empty-state"><div class="empty-icon"><i class="icon icon-calendar-x"></i></div><div><h3>还没有预约</h3><p>选一个方便的时间，与康复师确认下一次到店安排。</p></div></div>';
...
function showRecords() {
  modal('康复记录', `<div class="modal-list">${state.records.map((record) => `<article class="record-card"><time>${escapeHtml(record.date)}</time><h3>${escapeHtml(record.title)}</h3><p>${escapeHtml(record.content)}</p></article>`).join('')}</div><p class="auth-secondary-note">治疗记录由负责康复师填写；客户可查看与自身康复相关的记录摘要。</p>`);
}
...
function openWorkspace() {
  ...
  const therapistTabs = isTherapist
    ? `<span class="workspace-role-note"><i class="icon icon-shield-check"></i> 当前账号仅可查看本人日程与病人档案</span>`
  ...
  modal('治疗师工作台', `<div class="workspace-heading"><div><span class="section-kicker">TODAY · 08.23</span><h3>${escapeHtml(activeName)}的时间安排</h3></div><span class="workspace-date"><i class="icon icon-calendar-days"></i> 今天</span></div><div class="workspace-tabs">${therapistTabs}</div><div class="workspace-summary"><div><strong>${scopedSchedule.length}</strong><span>今日预约</span></div><div><strong>${pendingRecords}</strong><span>待补记录</span></div><div><strong>${scopedSchedule.filter((item) => item.tone !== 'pending').length}</strong><span>已确认 / 完成</span></div></div><p class="workspace-hint"><i class="icon icon-mouse-pointer-click"></i> 点击病人，查看主要诉求、治疗历史与康复进程。</p><div class="workspace-table">${rows || '<div class="workspace-empty">今天暂无这位康复师的安排。</div>'}</div><p class="workspace-note">日程可同步 Outlook Calendar；病人档案、治疗记录和评估结果建议以业务数据库作为唯一数据来源。</p>${adminAction}`, 'workspace');
}
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'mock-wechat-login') {
    ...
    if (auth.sensitiveConsent) recordConsent('health', 'granted', ['主诉', '评估结果', '疼痛评分', '治疗与训练记录'], '建立康复档案、治疗跟进与居家训练建议');
  ...
  if (action === 'grant-sensitive-consent') {
    ...
    recordConsent('health', 'granted', ['主诉', '评估结果', '疼痛评分', '治疗与训练记录'], '建立康复档案、治疗跟进与居家训练建议');
  ...
  if (action === 'revoke-sensitive-consent') {
    ...
    recordConsent('health', 'withdrawn', ['主诉', '评估结果', '疼痛评分', '治疗与训练记录'], '客户主动撤回健康信息新增处理授权');
  ...
  if (action === 'reset-demo') { config = { ...defaultConfig }; therapists = structuredClone(defaultTherapists); services = structuredClone(defaultServices); auth = { ...defaultAuth }; consentLog = []; patientConcernStore = {}; adminRange = 'day'; adminDate = '2026-08-23'; ['rehab-config', 'rehab-therapists', 'rehab-services', 'rehab-booking', 'rehab-records', 'rehab-auth', 'rehab-consent-log', 'rehab-training-done', 'rehab-patient-concerns', 'rehab-admin-range', 'rehab-admin-date'].forEach((key) => localStorage.removeItem(key)); state.booking = null; state.workspaceTherapist = 'li'; state.trainingDone = false; state.records = JSON.parse('[{"date":"2026.08.18","title":"第 4 次治疗 · 稳定训练","content":"颈肩紧张感较前减轻。继续进行胸椎活动度练习与核心激活，每天 12 分钟。"},{"date":"2026.08.11","title":"第 3 次治疗 · 动作重建","content":"久坐后腰背紧张评分由 6 降至 4，建议工作时每 45 分钟起身活动。"}]'); renderConfig(); renderAuth(); renderTherapists(); renderBooking(); renderArrival(); renderAdmin(); toast('已恢复示例数据'); }
});
